        ...
        function Graph(dataset, constraints) {
            ...
            function Resolve(originalPonits) {
                ...
                function using(threshold, evaluate) {
                    var count = 0;
                    for ( var i = 0; i < _this._originalPoints.length; i++) {
                        var currentPoint = _this._originalPoints[i];
                        if (evaluate(currentPoint[1],threshold)) {
                            if (i > 0
                                && (_this._data.length == 0 || _this._data[count - 1] == null)) {
                                _this._data[count++] = _this._getPointOnThreshold(threshold,_this._originalPoints[i - 1], currentPoint);
                            }
                            _this._data[count++] = currentPoint;
                        } else {
                            if (_this._data.length > 0 && _this._data[count - 1] != null) {
                                _this._data[count++] = _this._getPointOnThreshold(threshold,this._originalPoints[i - 1], currentPoint);
                                _this._data[count++] = null;
                                _this._data[count++] = null;
                            }
                        }
                    }
